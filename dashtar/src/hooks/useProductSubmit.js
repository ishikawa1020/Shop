...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!imageUrl) return notifyError("画像が必須です！");

      if (data.originalPrice < data.price) {
        setIsSubmitting(false);
        return notifyError(
          "セール価格は商品価格以下でなければなりません！"
        );
      }
      if (!defaultCategory[0]) {
        setIsSubmitting(false);
        return notifyError("デフォルトカテゴリーが必須です！");
      }
  ...
  const handleGenerateCombination = () => {
    if (Object.keys(values).length === 0) {
      return notifyError("まずバリアントを選択してください！");
    }
  ...
  const handleRemoveVariant = (vari, ext) => {
    ...
    swal({
      title: `よろしければ、この${ext ? "追加オプション" : "組み合わせ"}を削除します!`,
      text: `(よろしければ、この${ext ? "追加オプション" : "組み合わせ"}を削除します!)`,
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
  ...
  const handleIsCombination = () => {
    if ((isCombination && variantTitle.length) > 0) {
      swal({
        title: "本当にこの商品の組み合わせを削除しますか？",
        text: "(すべての組み合わせと追加オプションが削除されます。)",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      }).then((value) => {
  ...
  const handleQuantityPrice = (value, name, id, variant) => {
    ...
    if (name === "originalPrice" && Number(value) < Number(variant.price)) {
      ...
      notifyError("価格は元の価格以上でなければなりません！");
    ...
    if (name === "price" && Number(variant.originalPrice) < Number(value)) {
      // variants[id][name] = Number(variant.originalPrice);
      notifyError("セール価格は商品価格以下でなければなりません！");
