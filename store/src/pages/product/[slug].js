...
const ProductScreen = ({ product, attributes, relatedProducts }) => {
  ...
  const detailinfo = [
    ...
    product?.outquality,
    product?.remarks,
    product?.individualboxsize,
  ];
  const productdetil = ["本体サイズ	", "容量", "マーク", "生産国", "備考", "材質", "梱包形態", "入数（インナー）", "入数（アウター）", "備考", "個箱サイズ"];
                      ...
                      <div className=" w-3/5 xl:pr-6 md:pr-6  md:w-2/3 mob-w-full">
                        ...
                        <div>
                          <div className="text-sm leading-6 text-gray-500 md:leading-7">
                            ...
                            <br />
                            {/* Ensure product?.description is not undefined/null before calling Object.keys() */}
                            {product?.description && Object.keys(product.description).includes(lang)
                          ...
                          <div className="mt-2">
                            ...
                            <ul className="flex mt-4">
                              <li className="flex items-center text-center border border-gray-100 rounded-full hover: mr-2 transition ease-in-out duration-500">
                                <WhatsappShareButton
                                  url={``}
                                  quote=""
                                >
