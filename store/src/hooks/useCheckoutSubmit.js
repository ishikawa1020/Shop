...
const useCheckoutSubmit = (storeSetting) => {
    ...
    try {
      ...
      if (data.paymentMethod === "Cash") {
        ...
        const notificationInfo = {
          ...
          message: `${orderResponse?.user_info?.name}, さんが ${parseFloat(
            orderResponse?.total
          ).toFixed(2)} の注文をしました!`,
          image:
        ...
        const options = {
          ...
          handler: async function (response) {
            ...
            if (res) {
              ...
              notifySuccess("ご注文が確定しました！");
              Cookies.remove("couponInfo");
              localStorage.removeItem("products");
              emptyCart();

              await NotificationServices.addNotification({
                message: `${data?.firstName} ${total}ドルの注文を確定しました！`,
                orderId: res._id,
          ...
          modal: {
            ondismiss: function () {
              setTotal(total);
              setIsCheckoutSubmit(false);
              console.log("チェックアウトフォームが閉じられました！");
            },
          },
  ...
  const handleCouponCode = async (e) => {
    ...
    try {
      ...
      if (result.length < 1) {
        notifyError("有効なクーポンを入力してください！");
        return;
      }

      if (dayjs().isAfter(dayjs(result[0]?.endTime))) {
        notifyError("このクーポンは無効です！");
        return;
      }

      if (total < result[0]?.minimumAmount) {
        notifyError(
          `このクーポンを適用するには、最低 ${result[0].minimumAmount} USD が必要です！`
        );
        return;
      } else {
        notifySuccess(
          `クーポン ${result[0].couponCode} が ${result[0].productType} に適用されました！`
        );
