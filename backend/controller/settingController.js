...
const addStoreCustomizationSetting = async (req, res) => {
  try {
    ...
    res.send({
      ...
      message: "オンラインストアのカスタマイズ設定が正常に追加されました！",
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const getStoreCustomizationSetting = async (req, res) => {
  try {
    ...
    if (!storeCustomizationSetting) {
      return res.status(404).send({ message: "設定が見つかりません！" });
    }
...
const updateStoreCustomizationSetting = async (req, res) => {
  try {
    ...
    res.send({
      ...
      message: "オンラインストアのカスタマイズ設定が正常に更新されました！",
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
