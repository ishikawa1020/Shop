...
const addAttribute = async (req, res) => {
  try {
    ...
    res.send({
      message: "属性が正常に追加されました！",
    });
  } catch (err) {
    res.status(500).send({
      message: `属性の追加中にエラーが発生しました: ${err.message}`,
    });
  }
};
...
const updateStatus = async (req, res) => {
  try {
    ...
    res.status(200).send({
      message: `属性が正常に${newStatus === 'show' ? '公開されました' : '非公開になりました'}！`,
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
...
const updateChildStatus = async (req, res) => {
  try {
    ...
    res.status(200).send({
      message: `属性値が正常に${newStatus === 'show' ? '公開されました' : '非公開になりました'}！`,
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
