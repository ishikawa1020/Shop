...
const addCoupon = async (req, res) => {
  try {
    ...
    res.send({ message: "クーポンが正常に追加されました！" });
  } catch (err) {
...
const updateCoupon = async (req, res) => {
  try {
    ...
    if (coupon) {
      ...
      await coupon.save();
      res.send({ message: "クーポンが正常に更新されました！" });
    }
  } catch (err) {
    res.status(404).send({ message: "クーポンが見つかりません！" });
  }
};
...
const updateStatus = async (req, res) => {
  try {
    ...
    res.status(200).send({
      message: `クーポンが正常に${newStatus === 'show' ? '公開されました' : '非公開になりました'}！`,
    });
  } catch (err) {
    res.status(500).send({
      message: err.message,
